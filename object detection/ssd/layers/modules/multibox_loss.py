...
#         loss_c[pos]=0
        ...
        loss_c[pos] = 0  # filter out pos boxes for now  pos.size:(15,8732),将所有正例的loss_c设置为0了
#         loss_c = loss_c.view(num, -1)
        _, loss_idx = loss_c.sort(1, descending=True)
        _, idx_rank = loss_idx.sort(1)
#         num_pos = pos.long().sum(1, keepdim=True)
#         print('num_pose',num_pos,pos.size(1))
        num_neg = torch.clamp(self.negpos_ratio*num_pos, max=pos.size(1)-1)  #返回的size为(15,1),值为num_pose*3,不能超过最大值max
        neg = idx_rank < num_neg.expand_as(idx_rank)  #idx_rank.size:(15,8732)  num_neg.size:(15,1)，idx_rank里面是所有负例的loss排序，loss最大的值的位置为0
